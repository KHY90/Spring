...
public class JoinTests {
    ...
    * 2. 페치 조인 : Jpql에서는 성능 최적화를 위해 제공하는 기능으로 연관된 엔티티나 컬렉션을 한 번에 조회할 수 있다.
    ...
    * */
    ...
    public void 외부조인을_이용한_조회_테스트(){
        ...
        menuList.forEach(row -> {
            // obj[]을 하나씩 나누고 컬럼에 담음
            Stream.of(row).forEach(col -> System.out.println(col + " "));
            System.out.println();
        });
        // row는 menuList의 각 행을 나타냅니다. 즉, menuList의 각 요소입니다.
        // col은 row의 각 열을 나타냅니다. 즉, row의 각 요소입니다.
    }
    ...
    public void 페치조인을_이용한_조회_테스트(){
        // 페치 조인을 하면 처음 sql 실행 후 로딩할 때 조인결과를 다 조회한 뒤에 사용하는 방식이기 때문에 쿼리 실행 횟수가 줄어들게 된다.
        // 대부분의 경우 성능이 향상된다.
