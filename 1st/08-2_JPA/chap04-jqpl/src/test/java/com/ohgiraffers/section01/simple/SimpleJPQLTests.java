...
public class SimpleJPQLTests {
    ...
    * jpql은 엔티티 객체를 중심으로 개발 할 수 있는 객체 지향 쿼리이다.
    ...
    *   2-1 TypoedQuery : 반환할 타입을 명확하게 지정하는 방식일 때 사용하면 쿼리 객체의 메소드 실행을 결과로 지정하는 타입이 반환된다.
    *   2-2 Query : 반환할 타입에서 명확하게 지정할 수 없을 때 사용하며 쿼리 객체 메소드의 실행결과로 object 또는 object[]이 반환된다.
    *
    *  3. 쿼리 객체에서 제공하는 메소드 'getSingleResult()' 또는 'getResultList()'를 호출하여 쿼리를 실행하고 데이터베이스를 조회한다.
    *   3-1 getSinglResult() : 결과가 정확히 한 행일 경우 사용하며 없거나 많으면 예외가 발생한다.
    ...
    * */
    ...
    public void Query를_이용한_단일메뉴_조회_테스트(){
        ...
        Assertions.assertTrue(resultMenuName instanceof String); // resultMenuName의 타입이 string 타입과 같은지 검증
        Assertions.assertEquals("민트미역국", resultMenuName);
    }
    ...
    public void TypedQuery를_이용한_다중행_조회_테스트() {
        ...
        List<Menu> foundMenuList = query.getResultList(); // 리스트 형식으로 조회
