...
public class HandlerMapping {
    ...
    public Controller getController(String url) {
        ...
        try(ScanResult scanResult = new ClassGraph().enableAllInfo().acceptPackages("com.ohgiraffers.dispatcherservlet").scan()){
            // com.ohgiraffers.dispatcherservlet.controller.Controller 인터페이스를 상속받은 모든 클래스를 조회하고 classInfos에 저장함
            ...
            // classInfos의 요소를 순회함
            // 지금은 MainController, RedirectController, TestController가 가 순회됨
            for(Class<?> implClass : classInfos.loadClasses()){
                // 각 요소가 순회할 때 요소의 이름이 상위에서 정의한 type과 일치한다면
                if(implClass.getName().equals("com.ohgiraffers.dispatcherservlet.controller.Controller." + type)){
